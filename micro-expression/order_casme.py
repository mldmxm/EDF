...
if __name__ == "__main__":
    ...
    Y_PRED =[]
    people = os.listdir(r"./data_leaveone_all/CASME/")
    people.sort(key=lambda x: str(x))
    for _, one in enumerate(people):
        pic_class_train = os.listdir(r"./data_leaveone_all/CASME/" + one + '/u_train')
        pic_class_train.sort(key=lambda x: str(x))
        pic_class_test = os.listdir(r"./data_leaveone_all/CASME/" + one + '/u_test')
        pic_class_test.sort(key=lambda x: str(x))
        for i, pic_class in enumerate(pic_class_train):
            pic = os.listdir(r"./data_leaveone_all/CASME/" + one + '/u_train/' + pic_class)
            pic.sort(key=lambda x: str(x[:-4]))
            for m, one_pic in enumerate(pic):
                img = cv2.imread(
                    './data_leaveone_all/CASME/' + one + '/u_train/' + pic_class + '/' + one_pic)
                x_train.append(img)
                y_train.append(np.int(pic_class))

        for x, pictest_class in enumerate(pic_class_test):
            pic_test = os.listdir(r"./data_leaveone_all/CASME/" + one + '/u_test/' + pictest_class)
            pic_test.sort(key=lambda x: str(x[:-4]))
            for n, one_pictest in enumerate(pic_test):
                img = cv2.imread(
                    './data_leaveone_all/CASME/' + one + '/u_test/' + pictest_class + '/' + one_pictest)
                x_test.append(img)
